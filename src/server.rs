...
with a TCP stream "stream" for communication and a shared atomic flag "is_running_bool" to check if the server is running.
The old struct did not contain an atomic flag, which is used to check if the server is running or not.
*/
struct Client {
    stream: TcpStream,
    is_running_bool: Arc<AtomicBool>,
}
...
impl Client {
    ...
    pub fn new(stream: TcpStream, is_running_bool: Arc<AtomicBool>) -> Self {
        Client { stream, is_running_bool }
    }
    ...
    pub fn handle(&mut self) -> io::Result<()> {
        ...
        loop {
            if !self.is_running_bool.load(Ordering::SeqCst) {
...
impl Server {
    ...
    pub fn new(addr: &str) -> io::Result<Self> {
        ...
        let is_running_bool = Arc::new(AtomicBool::new(false));
    ...
    pub fn run(&self) -> io::Result<()> {
        self.is_running_bool.store(true, Ordering::SeqCst);
        ...
        while self.is_running_bool.load(Ordering::SeqCst) {
            match self.listener.accept() {
                Ok((stream, addr)) => {
                    info!("New client connected: {}", addr);

                    // This creates a new client instance and handles the client in a separate thread.
                    let is_running_bool = self.is_running_bool.clone();
                    thread::spawn(move || {
                        let mut client = Client::new(stream, is_running_bool);
    ...
    pub fn stop(&self) {
        if self.is_running_bool.load(Ordering::SeqCst) {// This checks if the server is running.
            self.is_running_bool.store(false, Ordering::SeqCst);
